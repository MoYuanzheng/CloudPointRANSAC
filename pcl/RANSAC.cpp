...
/*
	...
	返回值：
			...
			vector[3] => axis_direction.x ： 圆柱体轴方向的X坐标
			vector[4] => axis_direction.y ： 圆柱体轴方向的Y坐标
			vector[5] => axis_direction.z ： 圆柱体轴方向的Z坐标
			vector[6] =>     radius       :  圆柱体的半径
*/
std::vector<double> RANSAC_Cylinder(pcl::PointCloud<pcl::PointXYZ>::Ptr& cloud) {
	...
	seg.setRadiusLimits(3, 5);									// 设置圆柱模型半径的范围
...
/*
	...
	参数方程形式 => (x - center.x)^2 + (y - center.y)^2 = r^2 , center.z ∈ R
	返回值：
